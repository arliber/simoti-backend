...
from charlie import snippetsScore, tagsScore, snippetApplication
...
import config
from common.DAL import getSnippets, getArticleById, articlesBeforeDate
...
def charlie(articleId, publisherId, language):
  print('Charlie - charlie: working on {}/{} in [{}]'.format(publisherId, articleId, language))
  selectedSnippet = makeSnippetSelection(articleId, publisherId, language)
  print('Charlie - charlie: selected snippet ', selectedSnippet)
  if selectedSnippet is not None:
    ...
    return selectedSnippet['snippetId']
  else:
    return None

def charlieBulk():
  articles = articlesBeforeDate()
  print('Charlie - charlieBulk: working on [{}] articles'.format(len(articles)))
  matches = []
  for article in articles:
    matches.append({
      'articleId': article.key.id_or_name,
      'publisherId': article.key.parent.id_or_name,
      'snippetId': charlie(article.key.id_or_name, article.key.parent.id_or_name, 'he')
    })
